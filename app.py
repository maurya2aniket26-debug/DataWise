...
def init_db():
    ...
    # Add admin support to older databases
    user_columns = [row["name"] for row in conn.execute("PRAGMA table_info(users)").fetchall()]
    if "is_admin" not in user_columns:
        conn.execute("ALTER TABLE users ADD COLUMN is_admin INTEGER DEFAULT 0")

    # Default admin account. Change these values before deployment.
    admin_username = os.environ.get("DATAWISE_ADMIN_USERNAME", "admin")
    admin_password = os.environ.get("DATAWISE_ADMIN_PASSWORD", "admin123")
    admin_exists = conn.execute("SELECT id FROM users WHERE is_admin = 1 LIMIT 1").fetchone()
    if admin_exists is None:
        named_admin = conn.execute(
            "SELECT id FROM users WHERE username = ?",
            (admin_username,)
        ).fetchone()
        if named_admin:
            conn.execute(
                "UPDATE users SET is_admin = 1, password = ? WHERE id = ?",
                (generate_password_hash(admin_password), named_admin["id"])
            )
        else:
            conn.execute(
    ...
    # QUIZ ATTEMPTS - stores every quiz submission
    conn.execute("""
...
def quiz():
    ...
    if request.method == "POST":
        ...
        # Record every quiz submission for the Admin dashboard.
        conn = get_db()
        conn.execute(
            """
            INSERT INTO quiz_attempts (user_id, score, percentage)
            VALUES (?, ?, ?)
            """,
            (user_id, correct_count, percentage)
        )
        conn.commit()
        conn.close()
...
def admin():
    ...
    total_attempts = conn.execute("""
        ...
        WHERE u.is_admin = 0
    """).fetchone()["count"]
    completed_activities = conn.execute("SELECT COUNT(*) AS count FROM completed_activities").fetchone()["count"]
    pending_users = conn.execute("""
        SELECT COUNT(*) AS count FROM users u
        WHERE u.is_admin = 0 AND NOT EXISTS (
            SELECT 1 FROM completed_activities ca
            WHERE ca.user_id = u.id AND ca.activity = 'quiz'
        )
    """).fetchone()["count"]
    average_score = conn.execute("""
        SELECT COALESCE(ROUND(AVG(ps.score), 1), 0) AS value
        FROM privacy_scores ps JOIN users u ON u.id = ps.user_id
        WHERE u.is_admin = 0
    """).fetchone()["value"]
    highest_score = conn.execute("""
        SELECT COALESCE(MAX(ps.score), 0) AS value
        FROM privacy_scores ps JOIN users u ON u.id = ps.user_id
        WHERE u.is_admin = 0
    """).fetchone()["value"]
    conn.close()
    ...
    return render_template("admin.html", page="dashboard",
        ...
        completed_activities=completed_activities, pending_users=pending_users,
...
def admin_users():
    ...
    query = """
        SELECT u.id, u.username, u.created_at,
               COALESCE(ps.score, 0) AS privacy_score,
               CASE WHEN EXISTS (
                   SELECT 1 FROM completed_activities ca
                   WHERE ca.user_id = u.id AND ca.activity = 'quiz'
               ) THEN 1 ELSE 0 END AS quiz_completed
        FROM users u
        LEFT JOIN privacy_scores ps ON ps.user_id = u.id
        WHERE u.is_admin = 0
    """
    params = []
    if search:
...
def admin_delete_user(user_id):
    ...
    if user and not user["is_admin"]:
        conn.execute("DELETE FROM completed_activities WHERE user_id = ?", (user_id,))
        conn.execute("DELETE FROM quiz_attempts WHERE user_id = ?", (user_id,))
        conn.execute("DELETE FROM privacy_scores WHERE user_id = ?", (user_id,))
...
def admin_activities():
    ...
    conn = get_db()
    activities = []
    for key, label in activity_list:
        count = conn.execute("""
            SELECT COUNT(*) AS count
            FROM completed_activities ca JOIN users u ON u.id = ca.user_id
            WHERE ca.activity = ? AND u.is_admin = 0
        """, (key,)).fetchone()["count"]
        activities.append({"key": key, "label": label, "completed": count})

    users = conn.execute("""
        SELECT u.id, u.username, COALESCE(ps.score, 0) AS privacy_score,
               COUNT(ca.id) AS completed_count
        FROM users u
        LEFT JOIN privacy_scores ps ON ps.user_id = u.id
        LEFT JOIN completed_activities ca ON ca.user_id = u.id
        WHERE u.is_admin = 0
        GROUP BY u.id
    ...
    return render_template("admin.html", page="activities",
